...
class FFIWorkerProcess:
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                if cmd in ["tran_async", "op_async"]:
                    ...
                    # Handle explicit cleanup of async simulation resources
                    try:
    ...
    def _start_relay_thread(self, ffi_queue):
        ...
        # Ensure the shutdown event is in a non-signaled state for the new thread
...
class NgspiceIsolatedFFI(NgspiceBase):
    ...
    def tran_async(
        ...
        self._call_worker(
            "tran_async", tstep, tstop, *extra_args, throttle_interval=throttle_interval, disable_throttling=disable_throttling, fallback_sampling_ratio=fallback_sampling_ratio
        )
    ...
    def tran_async(self, tstep, tstop=None, *extra_args, throttle_interval: float = 0.1, disable_throttling: bool = False, fallback_sampling_ratio: int = 100, **kwargs):
        self._async_simulation_running = True
        self._call_worker("tran_async", tstep, tstop, *extra_args, throttle_interval=throttle_interval, disable_throttling=disable_throttling, fallback_sampling_ratio=fallback_sampling_ratio, **kwargs)
        return self.async_queue
