...
class NgspiceSubprocess(NgspiceBase):
    ...
    def tran_async(
        ...
        raise NotImplementedError(
            "Async transient analysis is not supported in subprocess backend. "
            "Use FFI or multiprocessing backend for async simulations."
        )

    def is_running(self) -> bool:
        """Check if async simulation is running."""
        return False  # Async simulation not supported in subprocess backend

    def safe_halt_simulation(
        self, max_attempts: int = 3, wait_time: float = 0.2
    ) -> bool:
        # Async simulation not supported in subprocess backend
        return True
    ...
    def get_sample_count(self) -> dict:
        ...
        """
        # Subprocess backend doesn't have direct sample tracking like FFI backends
        # Return default values for interface compatibility
        return {
            "samples_produced": 0,
            "total_data_points": 0,
            "sampling_rate": 0.0,
    ...
    def get_throttle_info(self) -> dict:
        ...
        """
        # Subprocess backend doesn't have throttling mechanism like FFI backends
        # Return default values for interface compatibility
        return {
